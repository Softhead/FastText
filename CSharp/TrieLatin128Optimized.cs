...
namespace CSharp
{
    // A trie with 128 bit nodes, optimized.
    //
    // Each 128 bit node contains:
    //
    // MSB
    // 4 bits: section and stop bits
    // 31 bits: the address of the next node in the trie, for section 0.
    // 31 bits: the address of the next node in the trie, for section 1.
    // 31 bits: the address of the next node in the trie, for section 2.
    // 31 bits: the address of the next node in the trie, for section 3.
    // LSB
    //
    // |------------------------128 bits----------------------------------------|
    // |-4b-||----31 bits----||----31 bits----||----31 bits----||----31 bits----|
    //
    ...
    public class TrieLatin128Optimized
    {
        ...
        private const int BitsToShiftSectionStopBits = 128 - 4;
        ...
        private static readonly UInt128[] AddressInPlaceBitMask =
            {
                ...
                UInt128.MaxValue - (((UInt128)MaximumAddress) << 62),
                UInt128.MaxValue - (((UInt128)MaximumAddress) << 31),
                UInt128.MaxValue - ((UInt128)MaximumAddress)
            };
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static uint GetAddressBySection(UInt128 value, int section)
        {
            return (uint)((value >> BitsToShiftAddress[section]) & AddressBitMask);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public UInt128 SetAddressBySection(UInt128 value, uint address, int section)
        {
            ...
            tempBits = ((UInt128)address) << BitsToShiftAddress[section];
            value &= AddressInPlaceBitMask[section];
            return value | tempBits;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static uint GetSectionStopBitsFromValue(UInt128 value)
        {
            return (uint)(value >> BitsToShiftSectionStopBits);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static UInt128 SetSectionStopBitsInValue(UInt128 value, uint sectionStopBits)
        {
            UInt128 tempBits = ((UInt128)sectionStopBits) << BitsToShiftSectionStopBits;
